...
"""
for g1 in milepostdf.itertuples():
    print(" {} , {} ".format( g1.SHAPE['x'],g1.SHAPE['y'] ))
    #if g1.SHAPE['x']==0 or g1.SHAPE['y']==0:
   
try:
    milepostdf['mp2xy'] = milepostdf['SHAPE'].apply(lambda g1 : 1 if (g1['x']==0 or g1['y']==0) else 0)
except Exception as e2:
    tend = datetime.today().strftime("%A, %B %d, %Y at %H:%M:%S %p")
    print(" Attempt to find Zero lat-lon coords failed {} - with error {}.\n in Dataframe {}  ".format (tend, e2,milepostdf))
    logger.error(" Attempt to find Zero lat-lon coords failed {} - with error {}.\n in Dataframe {}  ".format (tend, e2,milepostdf))
    
# In[40]:


milepostdf2 = deepcopy(milepostdf) # milepostdf[milepostdf['mp2xy'].eq(1)]
print("Data with zero,zero corrdinates \n {} , {} ".format(milepostdf2,milepostdf2['SHAPE']))
"""
...
for ix,frow in enumerate(milepostdf2.itertuples(),0):
    ...
    g1 = milepostdf2['SHAPE']
    if (g1['x']==0 or g1['SHAPE']['y']==0):
        fldrte= 'Route'
