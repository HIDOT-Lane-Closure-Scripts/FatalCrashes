...
if len(milepostdf)>0:
    ...
    for ix,frow in enumerate(milepostdf.itertuples(),0):
        ...
        if (abs(round(g1['x'],5))==0 and abs(round(g1['y'],5))==0):
            ...
            sdfrow = deepcopy(milepostdf.query("globalid=='{}'".format(frow.globalid))) #milepostdf[milepostdf.globalid=="'{}'".format(frow.globalid)]) # [milepostdf.globalid=="'{}'".format(frow.globalid)]
            sdfrow = (sdfrow.reset_index())
            #sdfrow = (deepcopy(frow))
            #sdfrow = pd.DataFrame(frow)
            #idxrow = sdfrow['index']
            print("conversion type : {} ; updated geometry : {} \n - cols : {} \n  dataframe : {}\n ".format(type(sdfrow),sdfrow.dtypes,sdfrow.columns,sdfrow))
            sdfrow.loc[0,'mp2xy'] =  x1
            sdfrow['SHAPE'] = sdfrow['SHAPE'].apply ( lambda x : gx)
            ...
            newfac = gac(sdfrow)  #, geometry_column='SHAPE')
            ...
            sdfrow = deepcopy(milepostdf.query("globalid=='{}'".format(frow.globalid))) # milepostdf[milepostdf.globalid=="'{}'".format(frow.globalid)]  # milepostdf.query("globalid=='{}'".format(frow.globalid))
            sdfrow.loc[:,'mp2xy'] =  x1
            newfac = gac(sdfrow)
            newfset = newfac.to_featureset()
            print ("updated : {} \n\n".format(newfset.to_dict()))
...
print("time : {} ; Data Frame fields  {} \n Point coords \n  Lon , Lat ".format(tbeg,milepostdf.columns))
milepostdf2 = deepcopy(milepostdf) # milepostdf[milepostdf['mp2xy'].eq(1)]

for g1 in milepostdf2.itertuples():
    if (round(g1.SHAPE['x'],5)==0 or round(g1.SHAPE['y'],5)==0):
        print("Zero Lat/Long at :  {} , Id : {} ; x: {} , y: {}".format(g1.casenumber,g1.globalid,g1.SHAPE['x'],g1.SHAPE['y'] ))
    elif (abs(round(g1.SHAPE['x'],5))>0 and abs(round(g1.SHAPE['y'],5))>0):
        #print("Case:  {} , Id : {} ; x: {} , y: {}".format(g1.casenumber,g1.globalid,g1.SHAPE['x'],g1.SHAPE['y'] ))
        x1 = 0
        gid = g1.globalid
        sdfrow = milepostdf2.query("globalid=='{}'".format(gid))
        sdfrow.loc[:,'mp2xy'] =  x1
        print("Oid : {} , gid ; {} , Crahsid : {} , x: {}  , y: {} ; geometry : {} \n  ".format(g1.objectid,gid,g1.casenumber,g1.SHAPE['x'], g1.SHAPE['y'],sdfrow['SHAPE']))
        newfset = gac(sdfrow).to_featureset()
        print ("updated : {} \n\n".format(newfset.to_dict()))
        resupdate = assyncedits(crashentrylyr,newfset) 
        print("result : {}".format(resupdate))
    else:
        print("something else at  {} , Id : {} ; x: {} , y: {}".format(g1.casenumber,g1.globalid,g1.SHAPE['x'],g1.SHAPE['y'] ))
        
    #if g1.SHAPE['x']==0 or g1.SHAPE['y']==0:
...
milepostdf2 = deepcopy(milepostdf) # milepostdf[milepostdf['mp2xy'].eq(1)]
x1=0
for ix,frow in enumerate(milepostdf2.itertuples(),0):
    print ("{}".format(frow))
    rteid = frow.Route
    bmpvalx = frow.milepost
    offset = frow.RteOffset
    ...
    if (abs(round(g1['x'],5))==0 and abs(round(g1['y'],5))==0):
        ...
        gx = rtesectpt(rtelyr,rteid,bmpvalx,offset,fldrte) # rtempt(rtelyr,rteFCSelNm,rteid,bmpvalx,offset,fldrte)
        print("type : {} ; recordset : {} \n - type {} - geom : {} \n\n ".format(type(frow),frow.SHAPE,type(gx),gx))
        sdfrow = milepostdf2.query("globalid=='{}'".format(frow.globalid))
        sdfrow.loc[:,'mp2xy'] =  x1
        sdfrow.loc[:,'SHAPE'] = gx
        #sdfrow[:,'SHAPE'] = sdfrow.apply(lambda x : gx,axis=1 )
        #sdfrow['mp2xy'] = sdfrow.apply(lambda x : 0,axis=1 )
        print("type : {} ; updated geometry : {} \n - shape col : {} \n\n ".format(type(sdfrow),sdfrow,sdfrow['SHAPE']))
        newfac = gac(sdfrow)
        newfset = newfac.to_featureset()
        print ("updated : {} \n\n".format(newfset.to_dict()))
        resupdate = assyncedits(crashentrylyr,newfset) 
        print("result : {}".format(resupdate))
    elif (abs(round(g1['x'],5))>0 and abs(round(g1['y'],5))>0):
        print("type : {} ; recordset : {} \n - type {} - geom : {} \n\n ".format(type(frow),frow.SHAPE,type(g1),g1))
        sdfrow = milepostdf2.query("globalid=='{}'".format(frow.globalid))
        sdfrow.loc[:,'mp2xy'] =  x1
        newfac = gac(sdfrow)
        ...
        print("No update due to Unusual coordinate : {} ; recordset : {} \n - type {} - geom : {} \n\n ".format(frow,frow.SHAPE,type(gx),gx))
...
milepostdf1 = milepostdf[milepostdf2['mp2xy'].eq(1)]
print("Data with zero,zero corrdinates \n {} , {} , x1 = {} ".format(milepostdf1,milepostdf1['SHAPE'],x1))
...
crashflds = [fd.name for fd in crashentrylyr.properties.fields]
print("{} ; Num recs : {} ; updated geometry : {} \n - shape head : {} \n\n ".format(tbeg,len(crasdfupdate),crashflds,crasdfupdate['SHAPE'].head()))
milepostdf1 = crasdfupdate[crasdfupdate['mp2xy'].eq(1)]
print("Data with zero,zero corrdinates \n {} , {} , x1 = {} ".format(milepostdf1,milepostdf1['SHAPE'],x1))
...
crashgeojson = crasdf.to_json(crashjsonpath)
...
MyAPIPw = "***" # Enter API Password here
...
crashreprev = socrashvw.revisions.create_update_revision()
print(crashreprev)
...
crashdropindf = deepcopy(crasdfupdate) 
crashupindex = crashdropindf.set_index('objectid',inplace=True)
crashdropindf.columns
...
crashupload = crashreprev.create_upload('crashes')
print(crashreprev)
crashuploadx = crashupload.df(crashdropindf) 
#print(crashuploadx)
...
socrashid = 'xr73-pg3t' # https://highways.hidot.hawaii.gov/Safety-Metric/Fatal-Crash-Alternate/xr73-pg3t
socrashvw = socrash.views.lookup(socrashid)
socrashconfig = 'FatalCrashData_03-21-2022_b444'

(crashrev, crashjob) = socrash.using_config('FatalCrashData_11-02-2020_0e39', socrashvw).df(crashdropindf)
# These next 2 lines are optional - once the job is started from the previous line, the
# script can exit; these next lines just block until the job completes
crashjob = crashjob.wait_for_finish(progress = lambda crashjob: print('Job progress:', crashjob.attributes['status']))
...
socrashconfig = 'FatalCrashData_03-21-2022_b444'
with open(crashcsvpath, 'rb') as csvfeats:
  (crashrev, crashjob) = socrash.using_config('FatalCrashData_11-02-2020_0e39', socrashvw).csv(csvfeats)
  # These next 2 lines are optional - once the job is started from the previous line, the
  # script can exit; these next lines just block until the job completes
  crashjob = crashjob.wait_for_finish(progress = lambda crashjob: print('Job progress:', crashjob.attributes['status']))
