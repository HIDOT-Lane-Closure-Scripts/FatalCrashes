...
logpath =  r"D:\MyFiles\HWYAP\crashes\logs" # r"I:\HI\AP\crash\logs" #
rptpath = r"D:\MyFiles\HWYAP\crashes\reports" # r"I:\HI\AP\crash\reports" #  
...
try:
    ...
    credentials = { 'userName' : 'dot_***', 'passWord' : '*******'} # Enter ArcGIS Credentials here
...
def rtesectpt(lyrts,rteid,bmpvalx,offs,fldrte='Route'):
    try:
        ...
        print("\nError {} processing {} Milepost {} , Offset {}".format(str(ex),rteid,bmpvalx,offset))
        logger.info("\nError {} processing {} Milepost {} , Offset {}".format(str(ex),rteid,bmpvalx,offset ))
...
FceSrcFSTitle = 'HiDOT Fatal Crash Entry' # arcpy.GetParameter(0) 
itypecrasrc= "Feature Service" # "Feature Layer" #   # "Service Definition"
fcrashnm = 'HiDOT Fatal Crash Entry'
...
logger.info("Title : {} Geometry Processing begins at {} ".format(FceSrcFSTitle,tbeg))                                                                                                                                                                                                                                                                                                                                                                                   
print("Title : {}  Geometry Processing begins at {} ".format(FceSrcFSTitle,tbeg))                                                                                                                                                                                                                                                                                                                                                                                   
...
print("Searching for Fatal Crash source {} from {} item for Service Title {} on AGOL...".format(itypecrasrc,portal_url,FceSrcFSTitle))
...
print ("Crash Query {} ; {} number of fatal crashes {} ".format(datesectqry,crasdf['LocMode'].value_counts(),len(crasdf)))
...
print ("number of fatalities entered with {} = {}".format(qrystr,len(milepostdf)))
...
#print("{}".format(milepostdf['SHAPE'].head(1)))
...
# Process geometry
geomilepostdf = deepcopy(milepostdf) 
for ix,frow in enumerate(milepostdf.itertuples(),0):
    try:
        ...
        if rteid=='340':
            print("\n processing {} Milepost {} , Offset {}".format(rteid,bmpvalx,offset))
...
import geojson
#logpath = r"I:\HI\AP\crash\logs" # r"D:\MyFiles\HWYAP\crashes\logs"
#rptpath = r"I:\HI\AP\crash\reports" #  r"D:\MyFiles\HWYAP\crashes\reports"
# crash layers
fcrashnm = 'HiDOTFatalCrashEntry'
crashentrysrc = crasrc.layers
crashentrylyr = lyrsearch(crashentrysrc, fcrashnm)
crashfeatset = crashentrylyr.query(where='1=1', out_fields='*')
crashfeats = crashfeatset.features
crasdf = crashfeatset.sdf
print ("crashdate : {} ; Feats : {}".format(crasdf['crash_date'].head(1),len(crashfeats)))
#crasdf= arcgis.features.SpatialDataFrame(data=crasdf)
#crashfeatset = crasdf.to_featureset()
print ("crashdate : {} ; Feats : {}".format(crashfeatset.features[0],len(crashfeatset)))
print ("crashdate : {} ; Feats : {}".format(crashfeatset.features[0],len(crashfeatset)))

crashrpath =rptpath # r"D:\MyFiles\HWYAP\crashes\reports" # r"C:\Work Database Data\FatalCrash\Data"
crashcsvname = "FatalCrashData.csv"
crashcsvpath = os.path.join(crashrpath, crashcsvname)
crashcsv = crasdf.to_csv(crashcsvpath)
crashgjsoname = "FatalCrashData.geojson"
crashgjsonpath = os.path.join(crashrpath, crashgjsoname)
crasdf = crashfeatset.sdf #crashentrylyr.query(where='1=1', out_fields='*',as_df=True)
crasgeojson = crashfeatset.to_geojson #crashentrylyr.query(where='1=1', out_fields='*',as_df=True)
#crashgeojson =crashfeats.to_geojson() #crashjsonpath) # 
#arcgis.features.FeatureLayerCollection.upload(crashgeojson,description="Fatal Crash Dataset")
#crashgeojson = crashfeats.save(rptpath,crashjsonname)
crashpname = "FatalCrashData.shp"
crashpath = os.path.join(crashrpath, crashpname)
#crashp = crasdf.save(crashpath)
crashpfile = crashfeatset.save(crashrpath, crashpname, encoding='utf8') # eatlnclrte.save('in_memory','rtesel')
print ("shapefile : {} ; Feats : {}".format(crashpfile,len(crashfeats)))
gjfile = open(crashgjsonpath, 'w')
gjfile.write(crasgeojson)
gjfile.close()
#with open(crashgjsonpath, 'w') as gjfile:
#   json.dump(crasgeojson, gjfile)
#crashgeojson = crashfeatset.save(crashrpath, crashjsoname, encoding='utf8') # eatlnclrte.save('in_memory','rtesel')
print ("GeoJSON file : {} ; geojson data : {} was created.".format(crashgjsonpath,crasgeojson))
logger.info ("GeoJSON file : {} ; geojson data : {} was created.".format(crashgjsonpath,crasgeojson))
...
MyAPIKey = "***" # Enter API Key here
MyAPIPw = "***" #Enter API Password
...
with open(crashgjsonpath, 'rb') as my_file:
  (revision, job) = socrata.using_config('FatalCrashData_12-04-2020_5b2c', view).geojson(my_file)
...
print("Title : {} Geometry Processing ends at {} \n".format(FceSrcFSTitle,tend))                                                                                                                                                                                                                                                                                                                                                                                   
logger.info("Title : {} Geometry Processing ends at {} \n".format(FceSrcFSTitle,tend))                                                                                                                                                                                                                                                                                                                                                                                   

"""
#Using ArcGIS CSV data replace final update successful


MyAPIKey = "***" #Enter API Key
MyAPIPw = "***" # Enter API Password
...
with open(crashcsvpath, 'rb') as my_file:
  (revision, job) = socrata.using_config('FatalCrashData_12-04-2020_e123', view).csv(my_file)
  # These next 2 lines are optional - once the job is started from the previous line, the
  # script can exit; these next lines just block until the job completes
  job = job.wait_for_finish(progress = lambda job: print('Job progress:', job.attributes['status']))
  sys.exit(0 if job.attributes['status'] == 'successful' else 1)
...
with open('FatalCrashData.geojson', 'rb') as my_file:
  (revision, job) = socrata.using_config('FatalCrashData_12-04-2020_fb9a', view).geojson(my_file)
  # These next 2 lines are optional - once the job is started from the previous line, the
  # script can exit; these next lines just block until the job completes
  job = job.wait_for_finish(progress = lambda job: print('Job progress:', job.attributes['status']))
  sys.exit(0 if job.attributes['status'] == 'successful' else 1)
